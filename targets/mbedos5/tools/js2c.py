...
HEADER = '''#include "jerryscript-mbed-util/js_source.h"
...
if args.mcu:
    ...
    if platform.system() == "Windows":
        ...
        try:
            cmd = "dir /s /b /AD \"{}\"".format("TARGET_MCU_" + args.mcu)
            intended_target = subprocess.check_output(cmd, shell = True, cwd=SRC_PATH, stderr=FNULL).strip()
        except subprocess.CalledProcessError:
            pass

        try:
            cmd = "dir /s /b \"{}\"".format("pins.js")
            js_targets = subprocess.check_output(cmd, shell = True, cwd=intended_target, stderr=FNULL).strip()

            js_target_list = [target for target in js_targets.split('\n')]
            if args.variant:
                js_target_list = [target for target in js_targets.split('\n') if args.variant in target]

            if len(js_target_list) > 1:
...
try:
    with open(os.devnull, "w") as dev_null:
        if platform.system() == "Windows":
            subprocess.check_call("where jshint", shell=True, stdout=dev_null)
        else:
...
if jshint_is_installed and platform.system() != "Windows":
    try:
